...
RESAMPLE_RULE = None   # "12H", "1D", "2D", "3D", "7D"
...
DPI = 300
SHOW_PLOTS = True
OUTPUT_FILENAME = "all_variants_complete_year_median_heat_demand_small_multiples.pdf"

VARIANT_STYLES: dict[str, dict[str, object]] = {
    "V1": {"color": "#4c78a8", "label": "V1"},
    "V2": {"color": "#f58518", "label": "V2"},
    "V3": {"color": "#54a24b", "label": "V3"},
    "V4": {"color": "#e45756", "label": "V4"},
    "V5": {"color": "#72b7b2", "label": "V5"},
    "V6": {"color": "#b279a2", "label": "V6"},
    "V7": {"color": "#ff9da6", "label": "V7"},
    "V8": {"color": "#9d755d", "label": "V8"},
}
...
def total_heat_kW(df: pd.DataFrame) -> pd.Series:
    ...
    Values are assumed to be W and converted to kW.
    """
...
def plot_small_multiples(median_by_variant: dict[str, pd.DataFrame]) -> Path:
    variants_to_plot = [v for v in SELECTED_VARIANTS if v in median_by_variant]
    if not variants_to_plot:
        raise RuntimeError("No profiles available for plotting.")

    n_plots = len(variants_to_plot)
    ...
    axes_flat = list(pd.Series(axes.ravel()))
    ...
    for i, variant in enumerate(variants_to_plot):
        ax = axes_flat[i]
        med_df = median_by_variant[variant]
        ...
        ax.set_title(str(style.get("label", variant)))
        ax.set_ylabel("Heating demand [kW]")
        style_axis(ax)
        format_year_axis(ax)

        all_start_dates.append(pd.to_datetime(med_df["datetime"].min()))
        all_end_dates.append(pd.to_datetime(med_df["datetime"].max()))
    ...
    for ax in axes_flat[:n_plots]:
        ax.set_xlim(min(all_start_dates), max(all_end_dates))
...
def main() -> None:
    ...
    for variant in VARIANT_ORDER:
        ...
        med_csv = OUTPUT_DIR / f"complete_year_{variant}_median_heat_demand_profile.csv"
    ...
    summary_csv = OUTPUT_DIR / "complete_year_all_variants_heat_demand_summary.csv"
    summary_df.to_csv(summary_csv, index=False)

    fig_path = plot_small_multiples(median_by_variant)
