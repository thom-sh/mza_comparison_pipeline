...
# Screenshot-inspired single-family palette.
# Lowest curves get stronger darker reds.
# Higher curves progressively move toward pale pink / light gray.
FILL_STEPS_LOW_TO_HIGH = [
    "#A63A3A",  # deepest muted red
    "#B84A4A",
    "#C85F5F",
    "#DB7E7E",
    "#F2A2A2",  # light pink-red
    "#DADADA",  # transition to gray like the reference image
    "#E6E6E6",
    "#F0F0F0",  # highest curves, very light
]

COMMON_LINE_COLOR = "#6E1F1F"
COMMON_LINE_WIDTH = 1.9
COMMON_FILL_ALPHA = 0.82
...
def ranked_fill_style(rank: int, n: int) -> tuple[str, float]:
    """
    rank = 0 -> lowest curve -> darkest fill
    rank = n-1 -> highest curve -> lightest fill
    """
    if n <= 1:
        return FILL_STEPS_LOW_TO_HIGH[0], COMMON_FILL_ALPHA

    palette_idx = int(round(rank * (len(FILL_STEPS_LOW_TO_HIGH) - 1) / (n - 1)))
    fill_color = FILL_STEPS_LOW_TO_HIGH[palette_idx]

    # Slightly reduce alpha for the higher curves so the stack does not get muddy.
    t = rank / (n - 1)
    alpha = COMMON_FILL_ALPHA - 0.18 * t
    return fill_color, max(0.45, alpha)
...
def plot_season_overlay(
    ...
    window_data: list[dict[str, object]] = []
    for variant in SELECTED_VARIANTS:
        ...
        plot_df = med_df.loc[
            (med_df["datetime"] >= start) & (med_df["datetime"] <= end)
        ].copy()
        if plot_df.empty:
            continue
    ...
    # Lowest curves first, highest curves last.
    draw_order = sorted(window_data, key=lambda item: float(item["mean_kW"]))

    ranked_colors: dict[str, dict[str, object]] = {}
    n_items = len(draw_order)
    for rank, item in enumerate(draw_order):
        variant = str(item["variant"])
        plot_df = item["df"]
        fill_color, fill_alpha = ranked_fill_style(rank, n_items)
        ranked_colors[variant] = {
    ...
    out_path = OUTPUT_DIR / f"all_variants_median_overlay_{label}_screenshot_palette.png"
