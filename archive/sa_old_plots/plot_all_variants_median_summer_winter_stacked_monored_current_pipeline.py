...
COMMON_LINE_WIDTH = 0.9
COMMON_FILL_ALPHA = 0.88

VARIANT_STYLES: dict[str, dict[str, object]] = {
    "V1": {"linestyle": "--", "linewidth": COMMON_LINE_WIDTH, "label": "V1: 1 zone"},
    "V2": {"linestyle": "-.", "linewidth": COMMON_LINE_WIDTH, "label": "V2: 1/storey + SC"},
    "V3": {"linestyle": "-", "linewidth": COMMON_LINE_WIDTH, "label": "V3: 2/floor + SC"},
    "V4": {"linestyle": (0, (7, 1)), "linewidth": COMMON_LINE_WIDTH, "label": "V4: 2/floor alt. + SC"},
    "V5": {"linestyle": ":", "linewidth": COMMON_LINE_WIDTH, "label": "V5: 4/floor + SC"},
    "V6": {"linestyle": (0, (5, 2)), "linewidth": COMMON_LINE_WIDTH, "label": "V6: 4/floor alt. + SC"},
    "V7": {"linestyle": (0, (3, 1, 1, 1)), "linewidth": COMMON_LINE_WIDTH, "label": "V7: 2/floor no SC"},
    "V8": {"linestyle": (0, (7, 2, 1, 2)), "linewidth": COMMON_LINE_WIDTH, "label": "V8: 2/floor heated SC"},
}
...
def total_heat_kW(df: pd.DataFrame) -> pd.Series:
    ...
    Current pipeline preference:
    1. Sum all zonal HeatDemand_<zone> columns, if available.
    2. Fall back to HeatDemand_SUM, if available.
    3. Fall back to HeatDemand, if available.

    HeatDemand values are assumed to be W and converted to kW.
    """
...
def plot_season_overlay(
    ...
    out_path = OUTPUT_DIR / f"all_variants_median_overlay_{label}_stacked_monored_current_pipeline.png"
