...
def plot_windowed_overlay(
    ...
    for variant in VARIANT_ORDER:
        ...
        # start = med_df["datetime"].min()
        start = pd.Timestamp("2021-11-17 00:00:00")
        end = start + duration
        plot_df = med_df.loc[(med_df["datetime"] >= start) & (med_df["datetime"] < end)].copy()
        if plot_df.empty:
            continue

        ax.plot(plot_df["datetime"], plot_df["median_heat_kW"], linewidth=1.0, label=variant)

    ax.set_title(f"Median total heating demand across all variants – {label.replace('_', ' ')}")
    ax.set_xlabel("Time")
    ax.set_ylabel("Total building heat demand [kW]")
    ax.grid(True, alpha=0.3)
    ax.legend(loc="best", ncol=2)

    out_path = OUTPUT_DIR / f"all_variants_median_overlay_{label}.png"
...
def main() -> None:
    ...
    for label, duration in WINDOW_SPECS:
        saved.append(plot_windowed_overlay(median_by_variant, label=label, duration=duration))
