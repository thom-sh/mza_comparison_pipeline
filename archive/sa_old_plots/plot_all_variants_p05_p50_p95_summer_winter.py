...
SEASON_SPECS: list[tuple[str, str, str]] = [
    # ("winter_q", "2021-01-01 00:00:00", "2021-03-31 23:00:00"),
    ("winter_q", "2021-10-01 00:00:00", "2021-12-31 23:00:00"),
    # ("summer_q", "2021-06-15 00:00:00", "2021-09-15 23:00:00"),
]
# ------------------------------------------------------------
...
def compute_variant_quantile_profile(df_profiles: pd.DataFrame) -> pd.DataFrame:
    ...
    q = (
        ...
        .quantile([LOW_Q, MID_Q, HIGH_Q])
        .unstack()
        .reset_index()
        .sort_values("datetime")
        .rename(columns={LOW_Q: "p05_heat_kW", MID_Q: "p50_heat_kW", HIGH_Q: "p95_heat_kW"})
    )
    return q[["datetime", "p05_heat_kW", "p50_heat_kW", "p95_heat_kW"]]
...
def _subset_period(q_df: pd.DataFrame, start: pd.Timestamp, end: pd.Timestamp) -> pd.DataFrame:
    return q_df.loc[(q_df["datetime"] >= start) & (q_df["datetime"] <= end)].copy()


def plot_season_overlay_all_three(
    ...
    for variant in VARIANT_ORDER:
        ...
        if q_df is None or q_df.empty:
            continue
        plot_df = _subset_period(q_df, start, end)
        if plot_df.empty:
            continue
        ...
        ax.plot(
            ...
            color=c,
            linestyle="-",
            linewidth=LINEWIDTH_P50,
            alpha=ALPHA,
            label=f"{variant} P50 (n_pr={counts_by_variant[variant]})",
        )
        ax.plot(
            plot_df["datetime"],
            plot_df["p95_heat_kW"],
    ...
    ax.set_title(f"P5, P50 and P95 total heating demand across all variants – {label}")
    ax.set_xlabel("Time")
    ax.set_ylabel("Total building heat demand [kW]")
    ax.grid(True, alpha=0.3)
    ax.legend(loc="best", ncol=2)

    out_path = OUTPUT_DIR / f"all_variants_p05_p50_p95_overlay_{label}.png"
    savefig(fig, out_path)
    return out_path


def plot_season_overlay_single(
    quantiles_by_variant: dict[str, pd.DataFrame],
    counts_by_variant: dict[str, int],
    label: str,
    start_str: str,
    ...
    ax.set_title(f"{quantile_label} total heating demand across all variants – {label}")
    ax.set_xlabel("Time")
    ax.set_ylabel("Total building heat demand [kW]")
    ax.grid(True, alpha=0.3)
    ax.legend(loc="best", ncol=2)
...
def main() -> None:
    ...
    for variant in VARIANT_ORDER:
        ...
        q_df.to_csv(OUTPUT_DIR / f"{variant}_p05_p50_p95_profile.csv", index=False)
    ...
    summary_df.to_csv(OUTPUT_DIR / "all_variants_p05_p50_p95_overlay_summary.csv", index=False)

    saved: list[Path] = []
    for label, start_str, end_str in SEASON_SPECS:
        saved.append(plot_season_overlay_all_three(quantiles_by_variant, counts_by_variant, label, start_str, end_str))
        saved.append(
            plot_season_overlay_single(
    ...
    for variant in quantiles_by_variant:
        print(OUTPUT_DIR / f"{variant}_p05_p50_p95_profile.csv")
    print(OUTPUT_DIR / "all_variants_p05_p50_p95_overlay_summary.csv")
    for path in saved:
        print(path)
