...
# One consistent colour per variant
VARIANT_COLORS = {
    "V2": "#4c78a8",
    "V3": "#f58518",
    "V5": "#54a24b",
...
def plot_quantile_panel(ax: plt.Axes, df_quant: pd.DataFrame, spec: dict, add_legend: bool = False) -> None:
    """Left panel: median bars with asymmetric P5-P95 error bars, coloured by variant."""
    ...
    bar_colors = [VARIANT_COLORS.get(v, COLOR_P50) for v in labels]
    ...
    if add_legend:
        handles = [
            Patch(facecolor=VARIANT_COLORS.get(v, COLOR_P50), edgecolor="none", label=v)
            for v in labels
        ]
        legend = ax.legend(handles=handles, title="Variant", frameon=True)
        legend.get_frame().set_facecolor("white")
        legend.get_frame().set_edgecolor(LEGEND_EDGE_COLOR)
        legend.get_frame().set_linewidth(0.8)
        legend.get_frame().set_alpha(1.0)
...
def plot_combined_figure(
    ...
    savefig(fig, OUTPUT_DIR / "granularity_v2_v3_v7_two_panel_bar_error_variant_colours.pdf")
...
def main() -> None:
    ...
    print(OUTPUT_DIR / "granularity_v2_v3_v7_two_panel_bar_error_variant_colours.pdf")
