...
OUTPUT_FILENAME = "selected_variants_combined_heat_profiles.png"
...
def main() -> None:
    ...
    for variant in SELECTED_VARIANTS:
        ...
        q_df.to_csv(OUTPUT_DIR / f"{variant}_quantile_profile_combined.csv", index=False)
    ...
    # optional: only show bands for selected variants
    # if you want bands for all plotted variants, use:
    # band_variants = set(variants_to_plot)
    band_variants = set(variants_to_plot)
    ...
    for variant in variants_to_plot:
        ...
        if variant in band_variants:
            ax.fill_between(
                ...
                q_df["q_high"],
                color=c,
                alpha=0.10,      # lower alpha = easier to see through
                linewidth=0,
                zorder=1,
            )

    # draw median lines on top
    for variant in variants_to_plot:
        q_df = quantiles_by_variant[variant]
        c = colors.get(variant, None)
    ...
    ax.set_title(f"Selected variants: median total heating demand with {band_label} bands")
    ax.set_xlabel("Time")
