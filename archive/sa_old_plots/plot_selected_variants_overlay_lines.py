...
# Average the 5 seeds first, then combine profiles across samples/buildings
AGGREGATE_SEEDS_FIRST = True

# Choose the single line statistic per time step: "mean" or "median"
PROFILE_STAT = "median"
...
OUTPUT_FILENAME = "selected_variants_combined_heat_profiles_no_quantiles.png"
...
def compute_representative_profile(df_profiles: pd.DataFrame) -> pd.DataFrame:
    ...
    if PROFILE_STAT == "median":
        out = df_work.groupby("datetime", dropna=False)["heat_kW"].median().reset_index(name="heat_kW")
    else:
        out = df_work.groupby("datetime", dropna=False)["heat_kW"].mean().reset_index(name="heat_kW")
...
def main() -> None:
    ...
    profiles_by_variant: dict[str, pd.DataFrame] = {}
    ...
    for variant in SELECTED_VARIANTS:
        ...
        rep_df = compute_representative_profile(df_profiles)

        if AGGREGATE_SEEDS_FIRST:
            n_profiles = int(
                aggregate_across_seeds_first(df_profiles)[["base_building_group", "weather_key", "sample_id"]]
                .drop_duplicates()
                .shape[0]
            )
        else:
        ...
        profiles_by_variant[variant] = rep_df
        counts_by_variant[variant] = n_profiles
        rep_df.to_csv(OUTPUT_DIR / f"{variant}_representative_profile_{PROFILE_STAT}.csv", index=False)

    variants_to_plot = [v for v in SELECTED_VARIANTS if v in profiles_by_variant]
    ...
    for variant in variants_to_plot:
        rep_df = profiles_by_variant[variant]
        ax.plot(
            rep_df["datetime"],
            rep_df["heat_kW"],
    ...
    stat_label = PROFILE_STAT.capitalize()
    ax.set_title(f"Selected variants: {stat_label} total heating-demand profiles")
    ax.set_xlabel("Time")
