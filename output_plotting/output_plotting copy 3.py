...
REGION_CSV = r"C:\WF\Thomas Sharon\Floorplan_Dataset\gml_msd\comparison_output\all_region_metrics.csv"
...
# ===============================================================
iou_sorted = np.sort(df_g["mean_iou_matched"].values)
...
y = df_g["mean_iou_matched"].values
...
# ===============================================================
plt.figure(figsize=(6, 4))
plt.hist(df_g["mean_iou_matched"], bins=15, edgecolor="black")
plt.xlabel("Mean IoU (matched regions)")
...
y = df_bubble["mean_iou_matched"].values
...
plt.xlabel("Region Recall")
plt.ylabel("Mean IoU (matched regions)")
plt.title("Recall vs Mean Matched IoU\n(Bubble size = #GT regions)")
plt.xlim(0, 1)
plt.ylim(0, 1)
plt.grid(True, alpha=0.3)
plt.tight_layout()
