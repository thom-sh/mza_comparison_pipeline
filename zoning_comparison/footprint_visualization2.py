import os
import numpy as np
import matplotlib.pyplot as plt
import pickle
import networkx as nx

from shapely.geometry import Polygon, Point
from shapely.ops import unary_union
from shapely.affinity import rotate as shp_rotate, translate as shp_translate

from utils import load_pickle
from constants1 import ROOM_NAMES


# ======================================================================
# SECTION 1 — Generic Geometry Utilities
# ======================================================================

def largest_poly(g):
    """Return the largest polygon from Polygon or MultiPolygon."""
    if g.geom_type == "Polygon":
        return g
    if g.geom_type == "MultiPolygon":
        return max(g.geoms, key=lambda p: p.area)
    return g.buffer(0)


def sample_polygon_boundary(poly, n_samples=200):
    """
    Sample N points uniformly along the polygon exterior.
    Used for Procrustes alignment.
    """
    poly = largest_poly(poly)
    ext = poly.exterior
    length = ext.length
    distances = np.linspace(0.0, length, num=n_samples, endpoint=False)
    pts = np.array([ext.interpolate(d).coords[0] for d in distances])
    return pts  # (N, 2)


def align_shape(poly, angle_deg, dx, dy, origin):
    """
    Apply a rigid transform to a polygon:
       1) rotate around origin
       2) translate
    """
    p = shp_rotate(poly, angle_deg, origin=origin, use_radians=False)
    p = shp_translate(p, xoff=dx, yoff=dy)
    return p


def align_pred(pred_fp, gt_fp, n_samples=200):
    """
    Align predicted footprint to GT footprint using 2D Procrustes-like alignment.
    Returns:
        pred_aligned (Polygon): aligned predicted footprint
        rot_deg (float): rotation applied in degrees
        dx (float): x-translation
        dy (float): y-translation
        origin (Point): rotation origin used
    """

    # 1) Sample boundary points (ensures same number of points, pure 2D)
    src = sample_polygon_boundary(pred_fp, n_samples)   # (N, 2)
    dst = sample_polygon_boundary(gt_fp, n_samples)     # (N, 2)

    # 2) Compute centroids of point clouds
    src_cent = src.mean(axis=0)
    dst_cent = dst.mean(axis=0)

    # 3) Center the points
    src_centered = src - src_cent
    dst_centered = dst - dst_cent

    # 4) Covariance matrix H (2×2)
    H = src_centered.T @ dst_centered

    # 5) SVD of H
    U, S, Vt = np.linalg.svd(H)

    # 6) Rotation matrix R (2×2)
    R = U @ Vt

    # Ensure proper orientation (avoid reflections / flipping)
    if np.linalg.det(R) < 0:
        U[:, -1] *= -1
        R = U @ Vt

    # 7) Extract rotation angle from R
    rot_rad = np.arctan2(R[1, 0], R[0, 0])
    rot_deg = np.degrees(rot_rad)

    # 8) Choose rotation origin = centroid of raw predicted footprint
    origin = pred_fp.centroid

    # Rotate the predicted footprint around origin
    pred_rot = shp_rotate(pred_fp, rot_deg, origin=origin, use_radians=False)

    # 9) Compute translation to match GT centroid
    pred_rot_cent = pred_rot.centroid
    gt_cent = gt_fp.centroid

    dx = gt_cent.x - pred_rot_cent.x
    dy = gt_cent.y - pred_rot_cent.y

    # 10) Translate rotated footprint
    pred_aligned = shp_translate(pred_rot, xoff=dx, yoff=dy)

    return pred_aligned, rot_deg, dx, dy, origin



# ======================================================================
# SECTION 2 — Ground Truth Extraction (Swiss)
# ======================================================================

def load_gt_rooms_and_footprint(gt_path):
    """Extract GT rooms and generate cleaned footprint."""
    G = load_pickle(gt_path)

    NAME_TO_IDX = {n: i for i, n in enumerate(ROOM_NAMES)}
    BALCONY_IDX = NAME_TO_IDX.get("Balcony", None)

    # Remove balconies
    if BALCONY_IDX is not None:
        G.remove_nodes_from(
            [n for n, d in G.nodes(data=True)
             if d.get("room_type") == BALCONY_IDX]
        )

    rooms = []
    room_types = []

    for _, d in G.nodes(data=True):
        geom = d.get("geometry")
        if geom:
            try:
                poly = Polygon(geom)
                if poly.is_valid and not poly.is_empty:
                    rooms.append(poly)
                    room_types.append(d.get("room_type"))
            except:
                pass

    merged = unary_union(rooms)
    fp = merged.buffer(0.5).buffer(-0.4)
    fp = largest_poly(fp.simplify(0.05, preserve_topology=True))
    return fp, rooms, room_types


def extract_gt_apartments(gt_path):
    """
    Extract connected apartment polygons and staircase polygons.
    """
    G = load_pickle(gt_path)

    NAME_TO_IDX = {n: i for i, n in enumerate(ROOM_NAMES)}
    PRIVATE_NAMES = ["Bedroom", "Livingroom", "Kitchen",
                     "Dining", "Bathroom", "Storeroom"]
    STAIRS_NAMES  = ["Stairs"]
    BALCONY_NAMES = ["Balcony"]

    PRIVATE_TYPES = {NAME_TO_IDX[n] for n in PRIVATE_NAMES if n in NAME_TO_IDX}
    STAIRS_TYPES  = {NAME_TO_IDX[n] for n in STAIRS_NAMES if n in NAME_TO_IDX}
    BALCONY_TYPES = {NAME_TO_IDX[n] for n in BALCONY_NAMES if n in NAME_TO_IDX}

    # Remove balconies
    balcony_nodes = [n for n, d in G.nodes(data=True)
                     if d.get("room_type") in BALCONY_TYPES]
    G.remove_nodes_from(balcony_nodes)

    # Remove entrance edges
    H = G.copy()
    entrance_edges = [(u, v)
                      for u, v, d in H.edges(data=True)
                      if d.get("connectivity") == "entrance"]
    H.remove_edges_from(entrance_edges)

    # Connected components → apartments
    components = list(nx.connected_components(H))
    apartments = []
    for comp in components:
        types = [G.nodes[n].get("room_type")
                 for n in comp if "room_type" in G.nodes[n]]
        if any(t in PRIVATE_TYPES for t in types):
            apartments.append(comp)

    # Create apartment polygons
    apartment_polys = []
    room_polys = []

    for _, d in G.nodes(data=True):
        geom = d.get("geometry")
        if geom:
            try:
                p = Polygon(geom)
                if p.is_valid and not p.is_empty:
                    room_polys.append(p)
            except:
                pass

    merged = unary_union(room_polys)
    footprint = merged.buffer(0.5).buffer(-0.4)
    if footprint.geom_type == "MultiPolygon":
        footprint = max(footprint.geoms, key=lambda g: g.area)
    footprint = footprint.simplify(0.05, preserve_topology=True)

    # Merge apartment polygons
    for comp in apartments:
        polys = []
        for n in comp:
            geom = G.nodes[n].get("geometry")
            if geom:
                try:
                    p = Polygon(geom)
                    if p.is_valid and not p.is_empty:
                        polys.append(p)
                except:
                    pass

        if polys:
            merged_apt = unary_union(polys)
            merged_apt = merged_apt.buffer(0.2).buffer(-0.2)
            if merged_apt.geom_type == "MultiPolygon":
                merged_apt = unary_union(merged_apt)
            apartment_polys.append(merged_apt)

    # Stairs
    stairs_polys = []
    for _, d in G.nodes(data=True):
        if d.get("room_type") in STAIRS_TYPES:
            geom = d.get("geometry")
            if geom:
                try:
                    p = Polygon(geom)
                    if p.is_valid:
                        stairs_polys.append(p)
                except:
                    pass

    return apartment_polys, stairs_polys, footprint


# ======================================================================
# SECTION 3 — Predicted Extraction
# ======================================================================

def load_predicted_zone_polygons(pred_path):
    """Extract predicted zone polygons."""
    with open(pred_path, "rb") as fh:
        bldg = pickle.load(fh)[0]

    storey1 = next(s for s in bldg["polygons"]["storeys"]
                   if s["storey_name"] == "Storey_1")

    zone_polys = []
    for zone in storey1["zones"]:
        polys = [
            f[0] for f in zone["floors"]
            if isinstance(f, tuple) and isinstance(f[0], Polygon)
        ]
        if polys:
            zone_polys.append(unary_union(polys))

    return zone_polys


def compute_predicted_footprint(pred_path):
    """Union of all predicted zones."""
    zones = load_predicted_zone_polygons(pred_path)
    if not zones:
        return None
    return largest_poly(unary_union(zones))


# ======================================================================
# SECTION 4 — VISUALIZATION
# ======================================================================

def visualize_alignment(ID, GT_BASE, PRED_PATH):
    gt_fp, _, _ = load_gt_rooms_and_footprint(GT_BASE)
    pred_fp = compute_predicted_footprint(PRED_PATH)
    if pred_fp is None:
        print(f"[WARN] no predicted footprint for building {ID}")
        return

    pred_aligned, rot, dx, dy, origin = align_pred(pred_fp, gt_fp)

    print("\n===================================================")
    print(f"🔥 Building {ID} – Procrustes Alignment Results")
    print("===================================================")
    print(f"GT area:               {gt_fp.area:.2f} m²")
    print(f"Predicted raw area:    {pred_fp.area:.2f} m²")
    print(f"Predicted aligned:     {pred_aligned.area:.2f} m²")
    print("---------------------------------------------------")
    print(f"Rotation applied:      {rot:.2f}°")
    print(f"Translation:           dx={dx:.2f}, dy={dy:.2f}")
    print("Origin (rotation):     ", origin)
    print("===================================================\n")

    fig, axs = plt.subplots(1, 3, figsize=(24, 8))
    polys = [gt_fp, pred_fp, pred_aligned]
    titles = ["Swiss GT", "Predicted (Raw)", "Predicted (Aligned)"]
    colors = ["#bfbfbf", "#ff7f7f", "#7fbf7f"]

    for ax, poly, title, col in zip(axs, polys, titles, colors):
        x, y = poly.exterior.xy
        ax.fill(x, y, color=col, alpha=0.85)
        ax.set_axis_off()
        ax.set_aspect("equal")
        ax.set_title(f"{title}\nArea = {poly.area:.1f} m²", fontsize=16)

    plt.suptitle(f"Building {ID} — GT vs Predicted Footprint Alignment",
                 fontsize=20)
    plt.tight_layout()
    plt.show()


def visualize_rooms_and_zones(ID, GT_BASE, PRED_PATH):
    apt_polys, stairs_polys, gt_fp = extract_gt_apartments(GT_BASE)
    pred_zones = load_predicted_zone_polygons(PRED_PATH)
    pred_fp = compute_predicted_footprint(PRED_PATH)
    if pred_fp is None:
        print(f"[WARN] no predicted footprint for building {ID}")
        return

    # SAME transform applied to footprint & all zones
    pred_aligned, rot, dx, dy, origin = align_pred(pred_fp, gt_fp)

    aligned_zones = [
        align_shape(z, rot, dx, dy, origin=origin)
        for z in pred_zones
    ]

    fig, axs = plt.subplots(1, 3, figsize=(27, 9))

    cmap_apts  = plt.get_cmap("tab10")
    cmap_zones = plt.get_cmap("tab10")

    # Panel 1 — GT apartments
    ax = axs[0]
    ax.set_title(f"Swiss GT Apartments ({len(apt_polys)})", fontsize=16)
    for i, apt in enumerate(apt_polys):
        xs, ys = apt.exterior.xy
        ax.fill(xs, ys, color=cmap_apts(i % 10), alpha=0.6, ec="black", lw=0.8)

    for poly in stairs_polys:
        xs, ys = poly.exterior.xy
        ax.fill(xs, ys, color="black", alpha=0.9)

    ax.set_aspect("equal")
    ax.set_axis_off()

    # Panel 2 — Predicted aligned zones
    ax = axs[1]
    ax.set_title("Predicted Zones (Aligned)", fontsize=16)
    for i, poly in enumerate(aligned_zones):
        xs, ys = poly.exterior.xy
        ax.fill(xs, ys, color=cmap_zones(i % 10), alpha=0.75, ec="black", lw=0.7)

    px, py = pred_aligned.exterior.xy
    ax.plot(px, py, "k--", lw=1.2)
    ax.set_aspect("equal")
    ax.set_axis_off()

    # Panel 3 — overlay
    ax = axs[2]
    ax.set_title("Overlay: GT vs Predicted (Aligned)", fontsize=16)
    gx, gy = gt_fp.exterior.xy
    ax.fill(gx, gy, color="gray", alpha=0.5, ec="none")
    ax.fill(px, py, color="green", alpha=0.5, ec="none")
    ax.set_aspect("equal")
    ax.set_axis_off()

    plt.suptitle(
        f"Building {ID} — Apartments vs Predicted Zones", fontsize=20
    )
    plt.tight_layout()
    plt.show()
