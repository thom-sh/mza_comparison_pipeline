...
if __name__ == "__main__":
    ...
    # Options:
    #   "selected" -> use manually selected IDs below
    #   "batch"    -> use full ID list from txt file
    ID_MODE = "batch"
    ...
    DATASET_CONFIG = {
        "rd": {
            "ground_truth_dir": REPO_DIR / "rd_ground_truth_extraction" / "data" / "ground_truth",
            "building_id_file": REPO_DIR / "rd_ground_truth_extraction" / "data" / "rd_thesis_building_ids.txt",
            "predicted_dir": REPO_DIR / "data" / "rd" / "rd_predicted_buildings" / "pkl",
            "output_dir": PROJECT_DIR / "output" / "rd",
        },
        "msd": {
            "ground_truth_dir": REPO_DIR / "msd_ground_truth_extraction" / "data" / "ground_truth",
            "building_id_file": REPO_DIR / "msd_ground_truth_extraction" / "data" / "msd_thesis_building_ids.txt",
            "predicted_dir": REPO_DIR / "data" / "msd" / "msd_predicted_buildings" / "pkl",
            "output_dir": PROJECT_DIR / "output" / "msd",
        },
    }
    ...
    if DATASET not in DATASET_CONFIG:
        raise ValueError("DATASET must be either 'rd' or 'msd'.")

    if ID_MODE == "batch":
        building_ids = load_building_ids(DATASET_CONFIG[DATASET]["building_id_file"])

    elif ID_MODE == "selected":
        building_ids = SELECTED_BUILDING_IDS_BY_DATASET[DATASET]

    else:
        raise ValueError("ID_MODE must be either 'batch' or 'selected'.")

    # ========================================================
    # SELECT DATASET-SPECIFIC PATHS
    # ========================================================

    GROUND_TRUTH_DIR = DATASET_CONFIG[DATASET]["ground_truth_dir"]
    PREDICTED_FOLDER = DATASET_CONFIG[DATASET]["predicted_dir"]
    OUTPUT_DIR = DATASET_CONFIG[DATASET]["output_dir"]

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    REGION_CSV = OUTPUT_DIR / f"all_region_metrics_{DATASET}.csv"
    GLOBAL_CSV = OUTPUT_DIR / f"all_global_stats_{DATASET}.csv"

    all_region_rows = []
    all_global_rows = []

    print("\n======================================")
    print(f"COMPARISON METRICS: {DATASET.upper()}")
    print("======================================")
    ...
    for bid in building_ids:
        print(f"\nProcessing {DATASET.upper()} building ID: {bid}")

        GT_BASE = GROUND_TRUTH_DIR / f"{bid}.pickle"
        PRED_PATH = PREDICTED_FOLDER / f"building_data_{bid}.pkl"
    ...
    append_or_create_csv(df_regions, str(REGION_CSV))
    append_or_create_csv(df_globals, str(GLOBAL_CSV))

    deduplicate_and_sort(str(REGION_CSV), str(GLOBAL_CSV))

    print("\n[DONE] Aggregated CSVs updated safely:")
    print(f"  - {REGION_CSV}")
    print(f"  - {GLOBAL_CSV}")
